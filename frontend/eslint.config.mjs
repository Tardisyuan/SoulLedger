...
// 间距节奏 = 规范 v1 §1.6(4 px 基数):{0.5:2 1:4 1.5:6 2:8 3:12 4:16 6:24 8:32 10:40 14:56}。
// 此前的 16(64px)不在规范里,全库也没有一处在用。
...
// 把 0.5 收进刻度后过期,已删。加一条是一个决定,要写理由。
    ...
    "spacing-rhythm": makeGuard("space", (raw, report, file) => {
      for (const { bare, chunk, at } of classTokens(raw)) {
        ...
        if (
          ...
          report(chunk, at, `\`${bare}\` 不在间距节奏里。允许的档位是 0.5/1/1.5/2/3/4/6/8/10/14(2/4/6/8/12/16/24/32/40/56px,规范 v1 §1.6),外加 0、px、auto`);
        }
      }
    }),
