...
export default function WelcomePage() {
  ...
  /**
   * Recent activity, from the audit log. It used to be three hard-coded rows.
   *
   * `setActivities([{ description: "新灵魂 张三 入库", user: "admin",
   ...
   * beneath the panel. It needs a session, which a public page may not have,
   * so the panel renders only for a signed-in visitor and simply does not
   * appear otherwise. Showing nothing is the honest answer to "what has been
   * happening?" when we are not allowed to know.
   */
  useEffect(() => {
    if (!user) return;
    let cancelled = false;
    // No `page_size` param: the backend runs a plain DRF PageNumberPagination
    // with no `page_size_query_param` (config/settings.py:201-203), so it would
    // be silently ignored — a parameter that reads as if it does something.
    // The page shows three; the first page is twenty; slice.
    auditApi
      .list()
      .then((res) => {
        if (cancelled) return;
        setActivities(
          res.data.results.slice(0, 3).map((entry) => ({
            id: entry.id,
            action: entry.action,
            description: entry.description,
            user: entry.user_display || entry.username || "",
            timestamp: entry.timestamp,
          }))
        );
      })
      .catch(() => {
        // A failed or forbidden audit read leaves the panel empty rather than
        // inventing rows. There is nothing here the operator can act on.
      });
    return () => {
      cancelled = true;
    };
  ...
  return (
    ...
    >
      <div className="space-y-6">
        ...
        {/* Quick Actions.
            The agent-status panel that used to sit beside this is gone. It
            rendered three invented agents ("soul-indexer", "ledger-decay",
            "judgment-assistant") with a pulsing green "运行中" dot — a live
            claim about processes that do not exist in this app — above a
            `href="/settings"` link to a route that does not exist either
            (`ls app/settings` → nothing). Its own comment said "In production,
            this would call the MCP tools", but it did not render a placeholder;
