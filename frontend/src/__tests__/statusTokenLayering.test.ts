 ...
 * The second half of this file (from "The keyboard focus ring" below) is the
 * same argument applied to `:focus-visible`: a load-bearing rule in globals.css
 * whose entire justification was a comment beside it. It pins that the rule
 * exists, that it is `!important`, that it names `--color-focus` rather than the
 * user-configurable `--color-accent`, that both themes declare that token, and
 * that it clears WCAG 1.4.11's 3:1 on every surface a focused control sits in
 * front of. It lives here rather than in a file of its own because it asks the
 * same question these do — WHICH TOKEN, not which colour — and reads the same
 * stylesheet through the same `support/globalsCssTokens` parser.
 *
 * No DOM and no browser: a directory walk, a brace count, and some arithmetic.
 */
...
//
// Same family of defect as everything above — a rule in globals.css whose only
// justification is a comment beside it — but the failure is louder. The ring is
