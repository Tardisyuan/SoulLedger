 ...
 * `:focus-visible` used to be pinned at the bottom of this file: the same
 * argument (a load-bearing rule in globals.css whose only justification was a
 * comment beside it) applied to the focus ring. It now lives in
 * `focusRingContract.test.ts`, which reads the same stylesheet through the
 * same `support/globalsCssTokens` parser. Nothing about the split changed
 * what either half asserts; the file had grown past the 500-line ceiling.
 *
