...
/** Mirrors the page's `tf`: falls back to the literal when a key is missing. */
const tf = (key: string, fallback: string) => fallback;
...
describe("RebirthFormSelect", () => {
  ...
  it("asks for the message keys that already exist, not new ones", () => {
    const spy = jest.fn((key: string, fallback: string) => fallback);
    renderSelect({ tf: spy });
    const keys = spy.mock.calls.map(([key]) => key);
    for (const form of SIX_PATHS) {
      expect(keys).toContain(`reincarnation.forms.${form}`);
    }
    expect(keys).not.toContain("reincarnation.forms.OTHER");
  });
