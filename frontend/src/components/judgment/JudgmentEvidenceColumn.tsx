...
export function JudgmentEvidenceColumn({ evidence }: { evidence: Record<string, unknown> }) {
  ...
  return (
    <section className="min-w-0">
      ...
      {entries.length === 0 ? (
        <p className="text-04 text-[oklch(var(--color-ink-subtle))] py-6">{t("judgment.no_evidence")}</p>
      ) : (
        <>
          <ol className="mt-4 divide-y divide-[oklch(var(--color-hairline))]">
            {entries.map(([key, value], index) => (
              <li key={key} className="grid grid-cols-[44px_1fr] gap-3 py-3">
                ...
                <span className="min-w-0">
                  <span className="text-04 text-[oklch(var(--color-ink))] block wrap-break-word">
                    <EvidenceValue value={value} />
                  </span>
                  {/* The source pointer: which key of the record this line is. */}
                  <span className="font-mono text-02 text-[oklch(var(--color-ink-subtle))] block mt-1 wrap-break-word">
                    {key}
                  </span>
                </span>
              </li>
          ...
          <details className="mt-4">
            <summary className="text-02 text-[oklch(var(--color-ink-muted))] cursor-pointer hover:text-[oklch(var(--color-ink))]">
              {t("judgment.view")}
            </summary>
            <pre className="mt-2 bg-[oklch(var(--color-surface-2))] p-3 font-mono text-02 text-[oklch(var(--color-ink))] overflow-auto max-h-64">
