...
export function SoulIdentityPanel({ soul }: { soul: QueueSoul }) {
  ...
  return (
    <section aria-labelledby="queue-identity-heading" className="border border-[oklch(var(--color-hairline))] bg-[oklch(var(--color-surface-1))] p-4">
      <h3 id="queue-identity-heading" className="text-2xs uppercase text-[oklch(var(--color-ink-muted))] mb-3">
        {t("judgment.queue.identity")}
      </h3>
      <div className="flex items-center gap-2 flex-wrap mb-3">
        <span className="text-md font-bold text-[oklch(var(--color-ink))]">{soul.name}</span>
      ...
      <dl className="grid grid-cols-2 gap-x-4 gap-y-2 text-sm">
        {rows.map((row) => (
          <div key={row.label}>
            <dt className="text-xs text-[oklch(var(--color-ink-muted))]">{row.label}</dt>
      ...
      {soul.description && (
        <p className="mt-3 text-sm text-[oklch(var(--color-ink-muted))] whitespace-pre-line">{soul.description}</p>
      )}
    </section>
  );
}
...
export function LedgerPanel({ ledger }: { ledger: QueueLedger }) {
  ...
  return (
    <section aria-labelledby="queue-ledger-heading" className="border border-[oklch(var(--color-hairline))] bg-[oklch(var(--color-surface-1))] p-4">
      <h3 id="queue-ledger-heading" className="text-2xs uppercase text-[oklch(var(--color-ink-muted))] mb-3">
      ...
      <div className="flex gap-6 mb-4">
        <div>
          <div className="text-xs text-[oklch(var(--color-ink-muted))]">{t("judgment.queue.merit")}</div>
          <Figure
            field="merit_score"
            quantity={SUMMARY_QUANTITIES.merit_score}
            t={t}
            className="font-mono tabular-nums text-md text-[oklch(var(--color-status-success))]"
          >
            {signed(ledger.merit_score)}
          </Figure>
        </div>
        <div>
          <div className="text-xs text-[oklch(var(--color-ink-muted))]">{t("judgment.queue.demerit")}</div>
          <Figure
            field="demerit_score"
            quantity={SUMMARY_QUANTITIES.demerit_score}
            t={t}
            className="font-mono tabular-nums text-md text-[oklch(var(--color-status-error))]"
          >
        ...
        <div>
          <div className="text-xs text-[oklch(var(--color-ink-muted))]">{t("judgment.queue.balance")}</div>
          {ledger.reading?.kind === "BALANCE" ? (
            <Figure
              field="karmic_balance"
              quantity={SUMMARY_QUANTITIES.karmic_balance}
              t={t}
              className="font-mono tabular-nums text-md text-[oklch(var(--color-ink))]"
            >
...
export function PriorCyclesPanel({ cycles }: { cycles: QueuePriorCycle[] }) {
  ...
  return (
    <section aria-labelledby="queue-cycles-heading" className="border border-[oklch(var(--color-hairline))] bg-[oklch(var(--color-surface-1))] p-4">
      <h3 id="queue-cycles-heading" className="text-2xs uppercase text-[oklch(var(--color-ink-muted))] mb-3">
...
export function RealmOptionsPanel({ realms }: { realms: QueueRealm[] }) {
  ...
  return (
    <section aria-labelledby="queue-realms-heading" className="border border-[oklch(var(--color-hairline))] bg-[oklch(var(--color-surface-1))] p-4">
      <h3 id="queue-realms-heading" className="text-2xs uppercase text-[oklch(var(--color-ink-muted))] mb-1">
      ...
      <p className="text-xs text-[oklch(var(--color-ink-subtle))] mb-3">{t("judgment.queue.realm_options_hint")}</p>
      {realms.length === 0 ? (
