    ...
    <div className={depth > 0 ? "ml-6 border-l-2 border-[oklch(var(--color-hairline))]/50 pl-4" : ""}>
      <div className="py-2">
        <div className="flex items-center gap-2 mb-1">
          <Link href={`/social/profile/${comment.author}`} className="text-03 font-medium text-[oklch(var(--color-ink))] hover:underline">
            {comment.author_name || comment.author_username}
          </Link>
          <span className="text-02 font-mono tabular-nums text-[oklch(var(--color-ink-muted))]">
            {formatDate(comment.create_time)}
          </span>
        </div>
        <p className="text-04 text-[oklch(var(--color-ink))] whitespace-pre-wrap">{comment.content}</p>
        <div className="flex items-center gap-3 mt-1">
          <button onClick={() => onReply(comment.id)} className="text-02 text-[oklch(var(--color-accent-ink))] hover:underline">
            Reply
          </button>
          ...
          {isAuthor && (
            <button
              ...
              className="text-02 text-[oklch(var(--color-ink-subtle))] hover:text-red-500 transition-colors"
            >
  ...
  return (
    <div>
      <form onSubmit={handleSubmit} className="mb-4">
        {replyTo && (
          <div className="flex items-center gap-2 mb-2 text-02 text-[oklch(var(--color-ink-muted))]">
        ...
        <div className="flex gap-2">
          <input
            ...
            className="flex-1 bg-[oklch(var(--color-surface-2))] border border-[oklch(var(--color-hairline))] px-3 py-2 text-04 text-[oklch(var(--color-ink))] placeholder-[oklch(var(--color-ink-subtle))] focus:outline-hidden focus:border-[oklch(var(--color-accent))]"
          />
          <button
            type="submit" disabled={!newComment.trim() || createComment.isPending}
            className="px-4 py-2 bg-[oklch(var(--color-accent))] hover:bg-[oklch(var(--color-accent))] text-03 font-medium transition-colors disabled:opacity-50"
          >
      ...
      {isLoading ? (
        ...
        <p className="text-04 text-[oklch(var(--color-ink-subtle))] text-center py-4">
