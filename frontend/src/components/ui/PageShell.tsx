 ...
 *    多数用处是按钮文字与正文小字,补上会把它们一起加粗 —— 和上面 `--text-md`
 *    那条是同一个论证。
 *
 *    **在 `<h2>` 这一层,这条规则目前由单一案例支撑。** 2026-09-10 扫 `app/`
 *    与 `src/components/` 的 38 个 `<h2>`,`text-sm` 的只有
 *    `app/notifications/page.tsx:277` 一个(其余 21 个 `text-2xs`、16 个
 *    `text-md`)。守卫只管 `<h2>`,所以守卫这一档确实只有一个样本。
 *
 *    **但这个排版本身不是孤例 —— 只是它的厚度比这里原先写的薄。**
 *    2026-09-10 同一天扫 `<h3>` 得到 `text-sm font-medium` 五处,当时这里记的是
 ...
 *    它们连 `text-ink-muted` 和 `mb-3` 都和队列面板那五个 `text-2xs uppercase`
 *    的组头逐字相同,只是写成了 13px medium。已按角色改成 `text-2xs uppercase`。
 *
 ...
 *    `blankComments`):**18 个 `<h3>`**,四档 —— `text-2xs uppercase`、
 *    `text-sm font-medium`、`text-sm`(4 处 semibold / 1 处 medium)、`text-md`,
 *    另有两处**一个 `text-0N` 都不写**。读上下文之后语义槽只有三个,而且**每一档
 *    都已有逐字符吻合 `<h2>` 的 `<h3>` 见证**:区块标签(队列面板的五处
 *    `text-2xs uppercase`)、面板标题(`workflow/page/TemplatePreview.tsx:145` 的
 *    `text-md`,它的行注释就写着「06 是区块标题那一档」)、列表行标题
 *    (organizations 与 RolesGrid 的两处 `text-sm font-medium`)。
 *
 ...
 *        是 `text-sm`,于是那个「标题」和它下面的说明段落同为 13px。两处补成
 *        面板标题档 `text-md` 并删掉空操作的 `font-semibold`。320px 浮层上放
 *        22px 标题不是新定的:同宽的 `SettingsDrawer` 抽屉标题本来就是
 *        `<h2 className="text-md">`。
 *      - `SettingsDrawer.tsx` 386/419/464 → `text-2xs uppercase`,见上一条。
 *      - **列表行标题这一档在 `<h3>` 上没定下来。** 3 处 `text-sm font-medium`
 *        对 5 处 `text-sm`,两边**没有结构分界线**:`RolesGrid.tsx:40` 与
 ...
 *      - `text-md` 上再写 `font-semibold` 是**空操作**(同一个 600),仓里有 5 处。
...
export interface PageShellProps {
  ...
  /**
   * 标题上方一行小字，`text-2xs font-mono uppercase`。
