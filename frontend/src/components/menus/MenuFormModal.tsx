      ...
      <form onSubmit={onSubmit} className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="flex flex-col gap-1">
            ...
            <span id={iconLabelId} className="text-01 uppercase text-[oklch(var(--color-ink-subtle))]">
              {t("menus.icon")} <span className="text-[oklch(var(--color-status-error))]">*</span>
            </span>
            <IconPicker
              value={form.icon}
            ...
            />
            {iconError ? (
              <p className="text-02 text-[oklch(var(--color-status-error))]">{t("menus.icon_missing_error")}</p>
            ) : (
              <p className="text-02 text-[oklch(var(--color-ink-tertiary))]">{t("menus.icon_required_hint")}</p>
            )}
            {iconCollision && (
        ...
        <div className="flex flex-col gap-1">
          <span id={rolesLabelId} className="text-01 uppercase text-[oklch(var(--color-ink-subtle))]">{t("menus.roles")}</span>
          ...
          <p className="text-02 text-[oklch(var(--color-ink-tertiary))]">{t("menus.roles_empty_hint")}</p>
        </div>
        <div className="flex flex-col gap-1">
          {isAdmin ? (
            ...
            <div className="flex items-center gap-2 text-03">
              <span className="text-[oklch(var(--color-ink-muted))]">{t("menus.active")}:</span>
              <span className={form.is_active ? "text-[oklch(var(--color-status-success))]" : "text-[oklch(var(--color-ink-subtle))]"}>
                {form.is_active ? t("menus.active") : t("menus.inactive")}
              </span>
            </div>
          )}
          <p className="text-02 text-[oklch(var(--color-ink-tertiary))]">
        ...
        <div className="flex flex-col gap-1">
          <div className="flex items-center gap-2">
            ...
            />
            <label htmlFor={visibleId} className="text-03 text-[oklch(var(--color-ink))]">{t("menus.visible_label")}</label>
          </div>
          <p className="text-02 text-[oklch(var(--color-ink-tertiary))]">{t("menus.visible_hint")}</p>
        </div>
