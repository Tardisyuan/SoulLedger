  ...
  return (
    ...
    <div className="bg-[oklch(var(--color-surface-1))] p-4 border border-[oklch(var(--color-hairline))]">
      ...
      <dl className="grid grid-cols-2 gap-4 text-03">
        ...
        <div>
          <dt className="text-[oklch(var(--color-ink-muted))]">{t("workflow.detail.case_type")}</dt>
          <dd className="text-[oklch(var(--color-ink))]"><DomainEnum namespace="workflow.case_types" value={workflow.case_type} /></dd>
        </div>
        <div>
          <dt className="text-[oklch(var(--color-ink-muted))]">{t("workflow.detail.judgment_verdict")}</dt>
          <dd className="text-[oklch(var(--color-ink))]"><DomainEnum namespace="workflow.verdicts" value={workflow.judgment_verdict} /></dd>
        </div>
        <div>
          <dt className="text-[oklch(var(--color-ink-muted))]">{t("workflow.detail.priority")}</dt>
          <dd className="text-[oklch(var(--color-ink))]">
        ...
        <div>
          <dt className="text-[oklch(var(--color-ink-muted))]">{t("workflow.detail.created_at")}</dt>
          <dd className="text-[oklch(var(--color-ink))]">{formatDateTime(workflow.created_at)}</dd>
        </div>
        <div>
          <dt className="text-[oklch(var(--color-ink-muted))]">{t("workflow.detail.completed_at")}</dt>
          <dd className="text-[oklch(var(--color-ink))]"><DomainText value={workflow.completed_at ? formatDateTime(workflow.completed_at) : null} missingKind={workflow.status === "COMPLETED" ? "unrecorded" : "inapplicable"} missingReason={statusLabel(workflow.status)} /></dd>
        </div>
      </dl>
    </div>
    </>
