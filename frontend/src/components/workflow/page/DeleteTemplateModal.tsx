  ...
  return (
    ...
    >
      <div className="space-y-3">
        <p className="text-sm text-[oklch(var(--color-ink))]">{t("workflow.delete_confirm_msg", { name: template?.name || "" })}</p>
        <p className="text-sm text-[oklch(var(--color-status-error))]">{t("workflow.delete_irreversible")}</p>
      </div>
    </BaseModal>
  );
}
