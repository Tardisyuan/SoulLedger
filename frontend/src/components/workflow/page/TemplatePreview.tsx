...
export function TemplatePreview({
  ...
  return (
    <>
      ...
      <div className="text-xs text-[oklch(var(--color-ink-subtle))] mb-3">
      ...
      {model.nodes ? (
        <div className="space-y-2 max-h-80 overflow-y-auto">
          {model.nodes.map((node, idx) => (
            <div key={idx} className="flex items-center gap-3 p-2 bg-[oklch(var(--color-surface-2))]">
              ...
              <span className="text-xs text-[oklch(var(--color-ink-muted))]">{node.court}</span>
              <span className="text-[oklch(var(--color-ink-subtle))]">·</span>
        ...
        <p className="text-xs text-[oklch(var(--color-ink-subtle))]">{t("workflow.view_to_see_nodes")}</p>
      )}
    </>
  );
}
