        ...
        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <span className="text-2xs uppercase text-[oklch(var(--color-ink-subtle))]">{t("souls.civilization")}</span>
              <p className="text-sm text-[oklch(var(--color-ink))] font-medium"><DomainEnum namespace="workflow.civilizations" value={template.civilization} /></p>
            </div>
            <div>
              <span className="text-2xs uppercase text-[oklch(var(--color-ink-subtle))]">{t("workflow.detail.case_type")}</span>
              <p className="text-sm text-[oklch(var(--color-ink))] font-medium">
                <DomainEnum namespace="workflow.case_types" value={template.case_type || template.caseType} />
              </p>
            </div>
          </div>
          <div>
            <span className="text-2xs uppercase text-[oklch(var(--color-ink-subtle))]">{t("workflow.detail.notes")}</span>
            <p className="text-sm text-[oklch(var(--color-ink))]"><DomainText value={template.description} /></p>
          </div>
          <div>
            <span className="text-2xs uppercase text-[oklch(var(--color-ink-subtle))]">{t("workflow.detail.nodes")}</span>
            <div className="mt-2 space-y-2 max-h-60 overflow-y-auto">
              ...
              {((template.nodes_json || template.nodes || []) as FlowNode[]).map((node: FlowNode, idx: number) => (
                <div key={idx} className="bg-[oklch(var(--color-surface-3))] p-2 text-sm">
                  <div className="font-medium text-[oklch(var(--color-ink))]">{node.node_name}</div>
              ...
              {(template.nodes_json || template.nodes || []).length === 0 && (
                <p className="text-sm text-[oklch(var(--color-ink-muted))]">{t("workflow.no_node_data")}</p>
              )}
            </div>
          </div>
        </div>
