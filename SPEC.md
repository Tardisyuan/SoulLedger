...
> **2026-08-28 追加的读法说明——本文是 2026-05-09 那一天的快照，不是现状。**
> 正文一律保留原样，因为改写一份带日期的设计稿只会毁掉它作为「当时是怎么想的」的
> 价值。已知与代码分叉的地方，读之前先知道这些：
>
> - **租户是四个，不是三个。** §1.1／§1.2 以及全文每一处「三租户」枚举
>   （§2 的架构图、§3 的 `Tenant.code` 说明、§5 的处置表、§7 的路由与 `tenantCode`、
>   §9 的验收项）都只列了 `CN_DIYU` / `EU_HEAVEN_HELL` / `EG_DUAT`。
>   希腊此后从欧洲租户里拆了出来（`realms/0018_split_greek_from_european`）：
>   `Civilization.GREEK` 是第四个枚举成员，租户码 `GR_HADES`，
>   由 `apps/souls/models.py::TENANT_CIVILIZATION` 单独映射。
>   权威清单是 `manage.py seed_tenants`，不是本文。
> - **§5 的业力／处置表已被 `backend/apps/ledger/readings.py` 取代。**
>   本文把三个体系当同一个 karma 数的三种阈值切法；代码明确拒绝了这条路，四个也一样——
>   中国是可抵消的累积账户，埃及是不可抵消的阈值检验，欧洲是罪责与补赎两个
>   互不相关的量，希腊给的是一段程序而不是一个量。
> - **条文语料在本文里根本不存在**，它是本文之后才有的东西：六个 corpus 共 172 条，
...
|| **voting_deadline** | datetime | **投票截止时间（创建后 7 天，可由 CHAIRMAN 延长）** |
|| voted_at | datetime | 投票时间（nullable） |
|| vote | enum | APPROVE / REJECT / ABSTAIN（nullable） |

**投票超时机制：**

- 每个参与者的 `voting_deadline` 默认为加入时间 + 7 天
- CHAIRMAN 可通过 API 延长截止时间
- 超时未投票的参与者自动标记为 `ABSTAIN`（弃权）
- 所有参与者投票或超时后，按多数票决出最终判决
- 自动弃权由 Celery 定时任务驱动（见 §5.5）
...
|| DISPATCHED | 已外派 | PENDING_DISPATCH（approve） | REINCARNATING（execute_dispatch）/ DISPOSED（recall） |
...
**触发条件：** 来源租户 TENANT_ADMIN 可在以下条件召回已外派灵魂：
- Soul.dispatch_status = DISPATCHED
- 目标租户尚未开始执行惩罚（即 DispatchRecord.status ≠ IN_PROGRESS）
- 目标租户尚未执行 `execute_dispatch()`

**召回流程：**
```
TENANT_ADMIN 发起 recall()
    │
    ├─ 验证：dispatch_status = DISPATCHED
    │         DispatchRecord.status ≠ IN_PROGRESS
    │
    ├─ 如满足条件：
    │    Soul.state → DISPOSED
    │    Soul.dispatch_status → null
    │    Soul.dispatched_to_tenant → null
    │    DispatchRecord.status → RETURNED
    │    记录 SOUL_RECALLED 事件
    │
    └─ 如不满足（惩罚已开始）：
         返回 409 Conflict："无法召回：惩罚已开始执行"
```

**注意：** 召回仅在惩罚**开始前**有效。一旦目标租户执行 `execute_dispatch()`，灵魂已进入 REINCARNATING 状态，无法召回。

### 4.6 并发控制（Concurrency Handling）

为防止跨租户并发操作导致状态不一致，所有状态转换采用以下机制：

**1. SELECT FOR UPDATE（数据库行锁）**

所有 Soul 状态转换必须在事务中使用 `SELECT FOR UPDATE`：

```python
from django.db import transaction

@transaction.atomic
def transition_soul_state(soul_id, from_state, to_state):
    soul = Soul.objects.select_for_update().get(id=soul_id)
    if soul.current_state != from_state:
        raise ConflictError(409, "状态已被其他操作修改")
    # 执行状态转换
    soul.current_state = to_state
    soul.save()
```
...
| 场景 | 冲突处理 |
|------|---------|
| 两个法官同时 conclude 同一 CrossTenantJudgment | 第一个写入成功，第二个返回 409 Conflict |
| 对已处于 PENDING_DISPATCH/DISPATCHED 状态的灵魂重复发起 dispatch | 返回 409："该灵魂已处于外派流程中" |
| 对非 DISPATCHED 状态发起 recall | 返回 409："无法召回：灵魂未处于已外派状态" |

**3. 原子性保证**

- `approve()` / `reject()` / `cancel()` / `recall()` / `execute_dispatch()` 均在原子事务中执行
- 事务内同时更新 Soul.state、Soul.dispatch_status、DispatchRecord.status
- 防止部分状态更新的非一致性问题
...
- 来源租户保留灵魂记录用于历史和审计
- **地狱类地域（realm_type=HELL）没有容量上限（无"满员"限制），可无限接收外派灵魂**
...
1. 每位参与者加入审判后自动获得 `voting_deadline` = 加入时间 + 7 天
2. CHAIRMAN 可通过 `POST /api/v1/cross-tenant-judgments/{id}/extend-deadline/` 延长截止时间
3. Celery 定时任务 `check_judgment_timeouts` 每 1 小时扫描所有 IN_REVIEW 状态的 CrossTenantJudgment
4. 对超过 `voting_deadline` 且未投票的参与者，自动设置 `vote = ABSTAIN, voted_at = now`
5. 当所有参与者完成投票（含自动弃权），判决自动按多数票计算：
   - APPROVE 票数 > REJECT 票数 → status = APPROVED
   - REJECT 票数 >= APPROVE 票数 → status = REJECTED
   - 所有票均为 ABSTAIN → 退回到 IN_REVIEW，需 CHAIRMAN 重新发起
6. Celery 任务定义示例：

```python
# backend/apps/dispatch/tasks.py
@app.task
def check_judgment_timeouts():
    from django.utils import timezone
    now = timezone.now()
    expired = CrossTenantJudgmentParticipant.objects.filter(
        cross_tenant_judgment__status='IN_REVIEW',
        vote__isnull=True,
        voting_deadline__lt=now
    )
    for p in expired:
        p.vote = CrossTenantJudgmentParticipant.Vote.ABSTAIN
        p.voted_at = now
        p.save()
        # 检查是否所有参与者都已投票
        judgment = p.cross_tenant_judgment
        if judgment.participants.filter(vote__isnull=True).count() == 0:
            approve_count = judgment.participants.filter(vote='APPROVE').count()
            reject_count = judgment.participants.filter(vote='REJECT').count()
            if approve_count > reject_count:
                judgment.status = 'APPROVED'
            else:
                judgment.status = 'REJECTED'
            judgment.concluded_at = now
            judgment.save()
```
...
### 6.0.1 错误响应格式（Error Response Schema）

所有 API 错误统一使用以下 JSON 格式：

```json
{
  "error": "ERROR_CODE",
  "message": "Human-readable description in the user's language",
  "details": {
...
|| 401 | UNAUTHORIZED | 未提供有效 JWT 令牌 |
|| 403 | FORBIDDEN | 缺少权限或跨租户访问被拒绝 |
|| 404 | NOT_FOUND | 资源不存在或不属于当前租户 |
|| 409 | CONFLICT | 状态转换冲突（如并发操作、不可转换的状态） |
|| 422 | UNPROCESSABLE_ENTITY | 业务逻辑拒绝（如 dispatch_enabled=false） |
...
// 400 Validation Error
{
  ...
  "details": {
    "soul_id": ["该字段是必填项"],
    "target_tenant_code": ["租户代码 'XX_UNKNOWN' 不存在"]
  }
}
...
|| **POST** | **/api/v1/dispatch/{id}/recall/** | **来源租户管理员召回已外派灵魂（仅限惩罚开始前）** | **TENANT_ADMIN（来源租户）** |
...
**POST /api/v1/dispatch/{id}/recall/ 请求体：**
```json
{
  "reason": "召回原因，如：因新证据出现需要重新审判"
}
```

**POST /api/v1/dispatch/{id}/recall/ 响应（成功 200）：**
```json
{
  "id": "uuid",
  "soul": {
  ...
  "status": "RETURNED",
  "recalled_at": "2026-05-09T12:00:00Z",
  "reason": "因新证据出现需要重新审判"
}
```

**POST /api/v1/dispatch/{id}/recall/ 响应（失败 409）：**
```json
{
  "error": "CONFLICT",
  "message": "无法召回：惩罚已开始执行",
...
### 7.6 联合审判 UI（Cross-Tenant Judgment UI）

跨租户联合审判页面（`/{tenant}/cross-judgments/[id]/`）设计如下：

**页面布局：**

```
┌─────────────────────────────────────────────────────────────────┐
│  ← 返回       跨租户联合审判庭    [语言: 中文(发起方)]    ⚙     │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  ┌─────────────────────────┐  ┌──────────────────────────────┐ │
│  │  审判概要                │  │  参与者（3 人）               │ │
│  │                         │  │                              │ │
│  │  灵魂: 张三              │  │  🧑 CN_DIYU    判官 (发起方) │ │
│  │  来源租户: CN_DIYU       │  │     APPROVE ✅              │ │
│  │  状态: IN_REVIEW         │  │                              │ │
│  │  创建时间: 2026-05-09    │  │  🧑 EU_HEAVEN  St.Peter     │ │
│  │                         │  │     ⏳ 待投票 (截止: 5/16)  │ │
│  │                         │  │                              │ │
│  │                         │  │  🧑 EG_DUAT    Anubis       │ │
│  │                         │  │     ABSTAIN ⚪              │ │
...
│  ┌─────────────────────────────────────────────────────────┐   │
│  │  投票操作（仅参与者可见）                                  │   │
│  │  [✅ 赞成 APPROVE]  [❌ 反对 REJECT]  [⚪ 弃权 ABSTAIN]  │   │
│  └─────────────────────────────────────────────────────────┘   │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```
...
**设计要点：**
- 联合审判页面以**发起方租户的语言**为主显示语言
- 所有参与者看到相同的审判页面内容，但操作按钮因角色而异
- **发起方法官**：可 conclude 审判（结束并判决）
- **参与者**（DISPATCH_JUDGE）：可投票（APPROVE/REJECT/ABSTAIN，一键操作）
- **CHAIRMAN**：可延长投票截止时间（extend deadline）
- 页面通过轮询（每 30 秒）或 WebSocket 更新投票状态
- 不内置聊天/讨论系统，法官们通过外部渠道沟通
- 投票截止时间以倒计时形式显示

### 7.7 应用内通知（In-App Notifications）

系统通过应用内通知（非 Email/推送）向用户传递重要事件：

**通知类型：**

|| 通知类型 | 触发事件 | 接收者 |
||---------|---------|--------|
|| CROSS_JUDGMENT_INVITED | 被邀请参与跨租户审判 | DISPATCH_JUDGE |
|| CROSS_JUDGMENT_VOTE_CAST | 其他参与者提交投票 | 所有审判参与者 |
|| CROSS_JUDGMENT_CONCLUDED | 审判结束 | 所有审判参与者 + 来源租户 JUDGE |
|| DISPATCH_PROPOSED | 新外派提议 | 目标租户 JUDGE |
|| DISPATCH_APPROVED | 外派被批准 | 来源租户发起者 |
|| DISPATCH_REJECTED | 外派被拒绝 | 来源租户发起者 |
|| SOUL_RECALLED | 灵魂被召回 | 目标租户 JUDGE |
...
class Notification(models.Model):
    ...
    notification_type = models.CharField(max_length=50)  # 如 CROSS_JUDGMENT_INVITED
    title = models.CharField(max_length=200)              # 简短标题
    body = models.TextField()                             # 详细信息
    is_read = models.BooleanField(default=False)
    related_url = models.CharField(max_length=500)        # 点击跳转的页面路径
    created_at = models.DateTimeField(auto_now_add=True)
```

2. **待处理列表（Pending List）**

前端在 NavBar 显示通知徽章（badge counter）：
- 未读通知数作为红色数字徽章显示在 🔔 图标上
- 点击 🔔 图标弹出下拉面板，显示最近 20 条通知
- 每条通知可点击跳转到关联页面（如 `/CN_DIYU/cross-judgments/{id}/`）
- "全部标为已读"按钮一键清除
...
4. **通知生成时机**：所有通知在 Celery 任务或 ViewSet 的 `perform_create` / `perform_update` 钩子中异步生成，不阻塞主请求。
