...
## 附录（2026-08-28）：§2 Limbo 一行的居民清单，库内已按史实分层

本节是**后加的**，不改上面任何一行。§2 罪罚对照表把 Limbo 居民写作「未受洗婴儿 +
有德异教徒（荷马、亚里士多德、萨拉丁、阿维森纳…）」并判 ✅——就「这些人都在
Inf. IV」而言仍然对，但四个名字不是一类人：荷马、亚里士多德是**生于基督之前**的
异教徒；萨拉丁（IV.129，卒 1193）、阿维森纳（IV.143，卒 1037）与阿威罗伊（IV.144）
生卒都在基督**之后**，是但丁**破例**安置于此的后世非基督徒。2026-08-27 考据审计
（commit `6ec90e5`）发现 seed 曾把整份名单笼统写成「生于基督之前」——紧跟其后列举
的名字里就有两个卒于基督之后，摘要句被它自己脚下的明细推翻。
`backend/apps/actors/mythology/statutes_inferno_entries.py` 的 EU-INF-C1 现已按两类
分开陈述。本报告 §6 的 24 处地点计数与 INFERNO corpus 的 26 条之别，仍照
`CORPUS_PROVENANCE["INFERNO"]` known_gap 的记载：两个数各数各的东西，互不改写。
