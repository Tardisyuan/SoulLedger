...
> **2026-09-26 注:这一句已过时。** 灵魂账号、`/me/`、转生申请、推送、朋友圈、书信(Matrix)与受刑计划都已在 `main` 上
> (见下文各处带日期的注)。仍未实施的是按租户分库(第 3 档)与各文明一个 homeserver 的联邦。
...
### 2026-09-17 跨文明调拨的性质(用户拍板)

- **调拨是暂居，不是迁籍。** 灵魂被调拨到另一文明受罚，处置执行完毕后**回归原文明**。
  此前代码实现的是单程(`dispatch` 执行时直接改 `soul.tenant`,不可撤销),与用户当初「先在 A 受罚、再到 B,由原租户发起」的设想不一致,以此条为准。
- 暂居期间，转生资格、转生申请与申诉一律按**原文明**计算(调拨前提交的申请照常可申诉);App 显示「暂居 X · 原属 Y」,不存在「往簿」这种屏。
  App 暂居显示规则:**皮肤跟当前所在文明;用词(词表)与转生资格跟原文明。**
- 本土的埃及、欧洲灵魂仍然没有转生(终局)。

### 2026-09-17 推送 / 朋友圈 / 聊天 与几件小事(用户拍板)

- **推送:** Expo Push Service。推送文案不含敏感内容(驳回理由全文、evidence、new_identity、密码)。
- **朋友圈:** 本文明(当前所在)内 + 关注;帖子四档可见性保留，但「公开」对灵魂也限本文明;官员不出现在灵魂朋友圈。审核：先发后审 + 举报，敏感词命中先进队列;官员可隐藏/删除/禁言 N 天，全部审计。
- **聊天:** 本轮单 Synapse homeserver 跑通(联邦留到按租户分库那一轮);灵魂用核心后端登录换 Matrix 登录。
  聊天对象：互关自由私聊;本文明内搜索到的非互关灵魂可发私聊请求，每 24 小时一条;灵魂可给所属殿司的官员收件箱发消息。~~跨文明灵魂之间不能私聊~~(**已被 2026-09-19 的决定取代**,见下节),官员不进灵魂间聊天。
- **React 版本:** Web 后台升到 React 19(等 App 界面分支合并后单独一轮)。**Android 暗色:** 跟随系统。
- **跨文明按钮:** 后端给出 `can_decide_cross_civilization`,前端不再依赖初审节点类型。
- App 的推送/朋友圈/聊天界面：等界面重做合并并补设计稿后再做;本轮只做后端与官员后台。
- **2026-09-17 同日：朋友圈与聊天(Matrix)用户决定暂停。** 已开工的进度以 WIP 提交保留在本地分支 `feat/soul-social`、`feat/soul-chat`(未验证、未推送、勿合并)。推送继续。

> **2026-09-26 注:上面两条的「暂停 / 等设计稿」已被后来的工作取代。** 书信(Matrix / Synapse)与朋友圈都已发货:
> App 书信标签 `cb73fa8f`(2026-09-19)、朋友圈第一 / 二轮 `9736be06` / `d73e932d`(2026-09-24)、
> 帖子图片 `ff6acda4`(2026-09-25,合并 `617784ea`);后端在 `apps/chat/`、`apps/social/`,
> App 屏在 `mobile/src/screens/{letters,conversation,circle,circlePeople,circleMedia}.tsx`。

### 2026-09-19 聊天规则改定(用户拍板)

用户原话:「所有灵魂理论上都该可以聊」「朋友圈搜索不要」。取代上节「跨文明灵魂之间不能私聊」一句(原文保留、划线)。

- **私聊不看文明。** 任意两个灵魂(本世账号)都可以私聊:互关 → 自由聊;非互关 → 私聊请求,每 24 小时一条,对方回过话或两人互关后解除。
  互关 = 双向关注,不论关注边记在哪个文明。
- **调拨、暂居、回归不改变灵魂之间的发言权。** 殿司收件箱仍按**当前所在**殿司:灵魂只能写给当前所在的殿司,调走后原殿司那封只读(官员仍可回)。
- **朋友圈搜索不改**,仍只到当前所在文明。私聊请求按 user_id 发起,~~不另加「跨文明找人」的入口~~。
  **同日补:加一个入口 —— 按完整灵魂编号精确查找**(`POST /api/v1/me/chat/lookup/`,跨文明;
  不存在 / 自己 / 前世 / 官员同一个 404;每个灵魂账号 20 次 / 小时)。查到的是朋友圈同一张名片
  (不回显编号),发起仍走 `POST /me/chat/conversations/` 的互关 / 24 小时规则。
- 禁言对私聊生效、被禁言仍可给殿司写信、转世关闭会话,照旧。
- 同日另定:非互关的私聊请求由后端「临时提权、以本人身份发、降回 0」,提权窗口由 Synapse 模块的一次性凭据挡住(不再由服务账号转发)。

### 2026-09-19 受刑计划落地(用户拍板见 `docs/ARCHITECTURE-sentence-plan.md` 决策记录 Q1–Q17、N1–N3、D1–D11)

取代上文「2026-09-17 跨文明调拨的性质」里「处置执行完毕后回归原文明」的**单段**读法:一个灵魂的处置是一份**有序的受刑计划**。

- **计划由结案产出。** 原属审判结案生成计划,第 1 站恒为原属;挂了跨文明联审的,各参与文明(联合审判官 / 主持)在联审里填**自己文明**那一站(界域、刑期),顾问不带站;永久刑期只能排最后。
- **每站执行完回原属,系统检查剩余站**:有则自动发起调拨去下一站,无则计划完成、开放转生申请(转生申请只在计划完成或撤销后开放)。**调拨仍是暂居**,是计划的搬运层;计划进行中官员不能手动调拨。
- **执行期间改判的三种情况**(当地加减项审判、已离开后追加请求、尚未到达时申请)都变成对计划的**请求**,**一律由原审判官批准**;重开审判批准后立即在原属开审,外地当前一站照常执行,刑满则在当地暂留等待。
- **撤销计划 = 赦免剩余刑期、视为完成**(`sentence_plan.cancel`,理由必填、写审计)。
- **分库约束照旧:** 三张新表 UUID 主键,执行地记 `tenant_code` 字符串、处置 / 调拨 / 审判记裸 UUID,不新增跨租户外键;计划归原属租户,执行地按「在计划上有节点」只读可见(`apps/core/tenant.py`,契约表 `tests/test_tenant_scoping_contract.py`)。
- **落地位置:** 后端 `apps/sentence_plan/`(阶段 1–3);Web 后台灵魂详情的「受刑计划」面板、「受刑请求」收件箱(`/sentence-requests`,菜单 `menus/0019`)、联审详情的各站填写与重排(阶段 4 Web)。
  **App 端**(Life 屏「我的受刑」、`sentence_*` 推送的落地页)等设计稿,尚未做;推送本身(`sentence_completed` / `sentence_waiting` / `sentence_amended` / `sentence_pardoned`)后端已发。
...
> **2026-09-26 注:下表是 2026-09-13(`e46eeb1`)的快照,不是今天的现状。** 至少这几行已被取代:
> 内置角色现在是 6 个,多了 `SOUL`(`backend/apps/authentication/models.py:56-67`);User 与 Soul
> 经 `apps/soul_accounts/models.py` 的 `SoulAccount`(`user` 一对一、`soul` 外键、链式 `previous_account`)关联;
> 灵魂自助界面在 `mobile/`(Expo App),接口在 `/api/v1/me/`;书信跑在 Synapse 上(`docker-compose.yml` 的 `synapse` 服务)。
...
> **2026-09-26 注:下面的「待决策」已决定并已实施** —— 选 Matrix(自建 Synapse),见文首
> 2026-09-14 决策记录与 2026-09-17 / 09-19 两节;本轮是单 homeserver(`docker-compose.yml` 的 `synapse`,
> `config/synapse/`),联邦留到按租户分库那一轮。下表保留为当时的比较。
