...
## 决策记录(2026-09-18 用户拍板;Q13–Q17、N1、N2、D1–D11 于 2026-09-19 用户确认)
...
| N1 | **(2026-09-19 用户已定)§11 N1 定为 (a)**:重开审判(REOPEN)结案产出**新裁决 + 一个新的原属节点**(灵魂回原属地后执行);约束「原属节点唯一且 `order=1`」相应放宽为「**首个节点是原属**」(§4.3)。 |
| N2 | **(2026-09-19 用户已定)§11 N2 定为 (a)**:请求方只能给**自己文明**加节点;要在别处加节点,由原属判官另开联审。 |
| D1 | **(2026-09-19 用户确认)撤销计划 = 赦免剩余刑期,视为完成**(否决「回到审判中重审」)。未开始的节点 CANCELLED、进行中的调拨取消,正在受的刑 ABORTED;灵魂在外则回归(`end_residence`,触发 `PLAN_CANCELLED`);然后走计划完成的同一条路径(可转世 → REINCARNATING,否则 SETTLED)。**终态沿用 CANCELLED**(官员看得出是刑满还是撤销),`eligibility` 把 COMPLETED 与 CANCELLED 都视为已完成。事件 `SENTENCE_PLAN_CANCELLED`,官员通知 `sentence_plan_cancelled`,灵魂推送 `sentence_pardoned`(只推给开放转生的,同 `sentence_completed`)。与正常完成一致:还有别的未结案审判时撤销答 409 `open_judgment`。取代 §3.1 原来「灵魂在外不自动回归」一句。 |
...
- **CANCELLED**:**赦免剩余刑期,视为完成**(决策记录 D1,2026-09-19 取代本句原文「灵魂若在外不自动回归」)。未执行节点 → CANCELLED,进行中的调拨记录 → CANCELLED,正在受的刑 → ABORTED,PENDING 请求 → WITHDRAWN,未结的重开审判撤案;灵魂在外则回归,然后与 COMPLETED 同一条路径进入可转生 / 终局。
...
| # | 问题 | 选项 | 推荐与代价 |
|---|---|---|---|
| **N1**(已定,2026-09-19:(a)) | 重开审判(REOPEN)结案产出什么 | (a) 新裁决 + 一个新的原属节点(灵魂回来后执行);(b) 只允许 `plan_changes`,不写裁决 | **(a)**,「再开一次审判」按字面是一次审判,有裁决。代价:「原属节点唯一且 `order=1`」放宽为「首个节点是原属」(§4.3 表第三行);(b) 与情况 1 的 AMENDMENT 除租户外无差别,那 REOPEN 就不必是独立 kind |
| **N2**(已定,2026-09-19:(a)) | 2.x 请求里 X 给**别的文明**加节点时,谁填那个节点的处置内容 | (a) 不允许:请求方只能加自己文明的节点;要加别处,由原属判官另开联审;(b) 允许,`realm_code` 留空,由该文明在 DISPATCHING 前补填,未补则 `advance` 停在该节点并通知 | **(a)**,与 Q1「各文明定自己节点」一致,且少一个「等待补填」状态。(b) 多一个中间态与一条通知 |
