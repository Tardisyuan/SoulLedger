...
UNCLAIMED_BY_DESIGN = {
    "dashboard": (
        "**两侧都没有消费者。** `dashboard.read` 授给了五个角色,而后端没有任何"
        "视图声明它(首页拼装的是别的族的数据),前端全仓 grep 也零命中 —— "
        "既不是后端闸门,也不是前端可见性开关。它是权限界面上一个不做任何事的"
        "开关。\n"
        "这一行此前写的是「前端首页的可见性开关」—— **那句是错的**,而它读起来"
        "像是已经核过。留着这一族要么给它一个消费者,要么从目录里删掉;"
        "在那之前,现状写在这里。"
    ),
    "system": (
