...
class EventType(models.TextChoices):
    ...
    # 刑期走完(`Disposition.expired_at` 被写下)。由每日的期满检查发,
    ...
    #
    # 专门的类型而不是 `STATE_CHANGED + payload.action`(用户 2026-09-18 拍板)。
    # 阶段 1 只声明,没有任何路径写入;阶段 2 起由 `SentencePlanService.advance` 发,
    # `SoulPushHandler` 按类型识别。都记在灵魂自己的时间线上,payload 不带理由与判官。
    SENTENCE_PLAN_CREATED = "SENTENCE_PLAN_CREATED"
    SENTENCE_NODE_ACTIVATED = "SENTENCE_NODE_ACTIVATED"
    SENTENCE_NODE_WAITING = "SENTENCE_NODE_WAITING"
    SENTENCE_NODE_COMPLETED = "SENTENCE_NODE_COMPLETED"
    SENTENCE_NODE_REFUSED = "SENTENCE_NODE_REFUSED"
    SENTENCE_PLAN_AMENDED = "SENTENCE_PLAN_AMENDED"
    SENTENCE_REQUEST_CREATED = "SENTENCE_REQUEST_CREATED"
