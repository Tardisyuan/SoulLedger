"""Data migration: 播种 `soul_inbox.read` / `soul_inbox.reply`,授予 ADMIN 与 MODERATOR。

与 0021 / 0022 同一写法与同一理由:落了 Permission 行,权限矩阵才能授予 / 收回;
授给 ADMIN 只是让矩阵显示的持有者与实际一致(checker 对 ADMIN 短路)。MODERATOR 的
授予与 `ROLE_PERMISSIONS` 一致(apps/perm/test_codename_coverage.py 钉住两者)。
...
class Migration(migrations.Migration):
    dependencies = [
        ("perm", "0023_social_moderate_permission"),
    ]
