...
阶段 1(本文件所在提交):只加模型,**不接推进**。计划在原属审判结案时生成,只含
原属节点;节点随处置执行被标记;其余状态(DISPATCHING / WAITING / RETRIAL / 请求)
在枚举里已就位,但阶段 2 之前没有任何路径写入它们。
...
class SentenceNode(AuditUserFields, models.Model):
    ...
    #: 冗余列,便于「原属节点有且只有一个且 order=1」这条约束。
    ...
    class Meta:
        ...
        constraints = [
            ...
            models.UniqueConstraint(
                fields=["plan"],
                condition=Q(is_deleted=False) & Q(is_home=True),
                name="unique_home_sentence_node",
            ),
            models.CheckConstraint(
                condition=Q(is_home=False) | Q(order=1),
